...
class LSTMModel(pl.LightningModule):
    ...
    def training_step(self, train_batch, batch_idx):
        review, sentiment = train_batch.review, train_batch.sentiment
        predictions = self.forward(review).squeeze(1)
        train_loss = self.criterion(predictions, sentiment)
        acc = self.binary_accuracy(predictions, sentiment)
    ...
    def validation_step(self, val_batch, batch_idx):
        review, sentiment = val_batch.review, val_batch.sentiment
        predictions = self.forward(review).squeeze(1)
        val_loss = self.criterion(predictions, sentiment)
        acc = self.binary_accuracy(predictions, sentiment)
    ...
    def test_step(self, test_batch, batch_idx):
        review, sentiment = test_batch.review, test_batch.sentiment
        predictions = self.forward(review).squeeze(1)
        test_loss = self.criterion(predictions, sentiment)
        acc = self.binary_accuracy(predictions, sentiment)
